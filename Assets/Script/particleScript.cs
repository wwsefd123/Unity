...
public class particleScript : MonoBehaviour {
    ...
    void Update()
    {
        ...
        objList.ForEach(delegate (GameObject obj)
        {
            ...
            {
                if (Vector3.Distance(inputXY[i], obj.transform.position) < 200f) // 키넥트 좌표와 거리가 200px 이하일 경우
                {
            ...
            //=======================================================
            // 마우스이벤트로 받은 좌표 처리
            if (Vector3.Distance(mousePos, obj.transform.position) < 200f) // 마우스 좌표와 거리가 200px 이하일 경우
            {
                // 마우스이벤트를 받을때
    ...
    IEnumerator CreatePositions()
    {
        ...
        yield return new WaitForSeconds(1);// 2초마다
        StartCoroutine("CreatePositions");
    }
        ...
        while (Socket_Thread_Flag)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < recvString.Length - 1; i++)
                {
                    ...
                    inputXY[i] = new Vector3(System.Convert.ToSingle(xylocation[0]) , -(System.Convert.ToSingle(xylocation[1]) ), 0f); // y값은 -를 해줘야함
                    inputCount = i + 1;
